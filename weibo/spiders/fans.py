...
class FansSpider(scrapy.Spider):
  name = "fans_list"
  def __init__(self, user_id, fans_num=10, *args, **kwargs):
    super(FansSpider, self).__init__(*args, **kwargs)
    self.allowed_domains = ["www.weibo.cn"]
    url_list = []
    for i in range(1, int(fans_num) + 1):
      url_list.append("http://weibo.cn/" + str(user_id) + "/fans?page=" + str(i))
  ...
  def parse(self, response):
    profile_list = response.selector.xpath('//body/div[@class="c"]/table')
